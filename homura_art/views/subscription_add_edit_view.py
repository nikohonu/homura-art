...
sources = [str(source.id) for source in Source.select()]


class SubscriptionAddEditView(QDialog, Ui_SubscriptionAddEditView):
    def __init__(
        self, subscription: dict = None, window_title="New subscription"
    ) -> None:
        super().__init__()
        self.setupUi(self)
    ...
    def load(self, subscription):
        if subscription:
            ...
            self.combo_box_source.setCurrentText(str(subscription["source"]))
    ...
    def result(self):
        return {
            ...
            "source": int(self.combo_box_source.currentText()),
        }
